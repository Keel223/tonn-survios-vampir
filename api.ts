...
export async function POST(req: Request) {
  try {
    ...
    // 2. Игрок создает заявку на вывод
    ...
    // 4. Админ: Автоматическая выплата TON из кошелька
    if (action === 'mark_paid') {
      if (tg_id !== ADMIN_ID) return NextResponse.json({ error: 'Forbidden' }, { status: 403 });
      ...
      const transfer = contract.createTransfer({
        ...
        messages: [internal({ to: targetReq.address, value: amountNano, bounce: false })]
      });
...
export async function GET() { return NextResponse.json({ status: 'API is working' }); }
